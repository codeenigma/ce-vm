...
# Start the project.
cd "$BUILD_DIR/ce-vm"
curl -O https://raw.githubusercontent.com/codeenigma/ce-vm-model/7.x/ce-vm/Vagrantfile

BRANCH=$(if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then echo $TRAVIS_BRANCH; else echo $TRAVIS_PULL_REQUEST_BRANCH; fi)
sed -i -- "s/ce_vm_upstream_branch = '7.x'/ce_vm_upstream_branch = '$BRANCH'/g" Vagrantfile
